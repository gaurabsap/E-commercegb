...
const Signup = () => {
    ...
    const SignupAccount = async(e) => {
        ...
        setLoad(true)
        try {
            const resq = await axios.post('/user/register', data)
            console.log(resq)
            setLoad(false)
            if(resq.status === 201){
                navigate('/login')
        ...
        <div className="signup">
            ...
            <form className='login_form'>
                    ...
                    <div className="cpassword">
                        ...
                        <input ref={cpass} type="password" placeholder='Enter your confirm-password' name='cpassword' onChange={handleInput}/>
                    </div>
                    <div className="checked_boxs">
                        <input type="checkbox" onChange={ShowPassword}/>
                        <label htmlFor="checkbox">Show password</label>
                    </div>
                    <p style={{color: 'red', textAlign: 'center', fontSize: '17px'}}>{error}</p>
                    <button onClick={SignupAccount}>Signup</button>
            </form>
